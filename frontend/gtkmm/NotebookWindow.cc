...
void NotebookWindow::add_action_to_actiongroup(
	Glib::RefPtr<Gtk::Action> action,
	void(NotebookWindow::* callback)(void),
	const char* accel_key)
{
	if (callback) {
		auto wrapped_callback = sigc::mem_fun(*this, callback);
		if (accel_key)
			actiongroup->add(action, Gtk::AccelKey(accel_key), wrapped_callback);
		else
			actiongroup->add(action, wrapped_callback);
	}
	else {
		if (accel_key)
			actiongroup->add(action, Gtk::AccelKey(accel_key));
		else
...
Glib::RefPtr<Gtk::Action> NotebookWindow::menu_add_action(
	const char* name,
	const char* label,
	void(NotebookWindow::* callback)(void),
	const char* accel_key)
{
	auto action = Gtk::Action::create(name, label);
	add_action_to_actiongroup(action, callback, accel_key);
	return action;
}

Glib::RefPtr<Gtk::Action> NotebookWindow::menu_add_action(
	const char* name,
	Gtk::BuiltinStockID label,
	void(NotebookWindow::* callback)(void),
	const char* accel_key)
{
	auto action = Gtk::Action::create(name, label);
...
Glib::RefPtr<Gtk::Action> NotebookWindow::menu_add_action(
	...
	const char* accel_key)
{
	auto action = Gtk::Action::create(name, stockid, label);
	add_action_to_actiongroup(action, callback, accel_key);
	return action;
}

Glib::RefPtr<Gtk::RadioAction> NotebookWindow::menu_add_radioaction(
	const char* name,
	const char* label,
	Gtk::RadioAction::Group& group,
	int value,
	void(NotebookWindow::* callback)(Glib::RefPtr<Gtk::RadioAction>),
	bool active,
	const char* accel_key)
{
	auto action = Gtk::RadioAction::create(group, name, label);
	action->property_value() = value;
	action->set_active(active);

	auto bound_callback = sigc::bind(sigc::mem_fun(*this, callback), action);

	if (accel_key)
		actiongroup->add(action, Gtk::AccelKey(accel_key), bound_callback);
	else
		actiongroup->add(action, bound_callback);

	return action;
}

void NotebookWindow::setup_menus()
{
	actiongroup = Gtk::ActionGroup::create();

	auto menu_file = menu_add_action("MenuFile", "_File");
		menu_add_action("FileNew", Gtk::Stock::NEW, &NotebookWindow::on_file_new, "<ctrl>N");
		menu_add_action("FileOpen", Gtk::Stock::OPEN, &NotebookWindow::on_file_open, "<ctrl>O");
		menu_add_action("FileClose", Gtk::Stock::CLOSE, &NotebookWindow::on_file_close, "<ctrl>W");
		menu_add_action("FileSave", Gtk::Stock::SAVE, &NotebookWindow::on_file_save, "<ctrl>S");
		menu_add_action("FileSaveAs", Gtk::Stock::SAVE_AS, &NotebookWindow::on_file_save_as, "<ctrl><shift>S");
		menu_add_action("FileExportHtml", "Export to standalone HTML", &NotebookWindow::on_file_export_html);
		menu_add_action("FileExportHtmlSegment", "Export to HTML segment", &NotebookWindow::on_file_export_html_segment);
		menu_add_action("FileExportLaTeX", "Export to standalone LaTeX", &NotebookWindow::on_file_export_latex);
		menu_add_action("FileExportPython", "Export to Python/Cadabra source", &NotebookWindow::on_file_export_python);
		menu_add_action("FileQuit", Gtk::Stock::QUIT, &NotebookWindow::on_file_quit);

	auto menu_edit = menu_add_action("MenuEdit", "_Edit");
		menu_add_action("EditUndo", Gtk::Stock::UNDO, &NotebookWindow::on_edit_undo, "<ctrl>Z");
		action_copy = menu_add_action("EditCopy", Gtk::Stock::COPY, &NotebookWindow::on_edit_copy, "<ctrl>C");
		action_copy->set_sensitive(false);
		action_paste = menu_add_action("EditPaste", Gtk::Stock::PASTE, &NotebookWindow::on_edit_paste, "<ctrl>V");
		menu_add_action("EditInsertAbove", "Insert cell above", &NotebookWindow::on_edit_insert_above, "<alt>Up");
		menu_add_action("EditInsertBelow", "Insert cell below", &NotebookWindow::on_edit_insert_below, "<alt>Down");
		menu_add_action("EditDelete", "Delete cell", &NotebookWindow::on_edit_delete, "<ctrl>Delete");
		menu_add_action("EditSplit", "Split Cell", &NotebookWindow::on_edit_split, "<ctrl>Return");
		menu_add_action("EditMakeCellLaTeX", "Cell is LaTeX", &NotebookWindow::on_edit_cell_is_latex, "<ctrl><shift>L");
		menu_add_action("EditMakeCellPython", "Cell is Cadabra/Python", &NotebookWindow::on_edit_cell_is_python, "<ctrl><shift>P");
		menu_add_action("EditIgnoreCellOnImport", "Ignore cell on import", &NotebookWindow::on_ignore_cell_on_import, "<ctrl><shift>I");


	auto menu_view = menu_add_action("MenuView", "_View");
		menu_add_action("ViewSplit", "Split view", &NotebookWindow::on_view_split);
		menu_add_action("ViewClose", "Close view", &NotebookWindow::on_view_close);
		menu_add_action("MenuConsoleVisibility", "Console");
			Gtk::RadioAction::Group group_console;
			menu_add_radioaction("ConsoleHide", "Hide", group_console, 0, &NotebookWindow::on_view_set_console_vis, true);
			menu_add_radioaction("ConsoleDock", "Dock", group_console, 1, &NotebookWindow::on_view_set_console_vis);
			menu_add_radioaction("ConsoleFloat", "Float", group_console, 2, &NotebookWindow::on_view_set_console_vis);
		menu_add_action("MenuStyle", "Style");
			menu_add_action("StyleFont", "Font...", &NotebookWindow::on_view_set_font);
			auto text_colour = menu_add_action("StyleTextColour", "Text Colour...", &NotebookWindow::on_view_set_text_colour);
			text_colour->set_sensitive(!prefs.highlight);
			menu_add_action("MenuSyntaxHighlighting", "Syntax Highlighting");
				Gtk::RadioAction::Group group_syntax_hl;
				menu_add_radioaction("SyntaxHighlightingOn", "On", group_syntax_hl, 1,
					&NotebookWindow::on_view_set_syntax_hl, prefs.highlight);
				menu_add_radioaction("SyntaxHighlightingOff", "Off (default)", group_syntax_hl, 0,
					&NotebookWindow::on_view_set_syntax_hl, !prefs.highlight);
				menu_add_action("SyntaxHighlightingCustomize", "Customize...", &NotebookWindow::on_view_customize_syntax_hl);
			menu_add_action("MenuSelectionColour", "Selection Colour");
				Gtk::RadioAction::Group group_sel_colour;
				menu_add_radioaction("SelectionColourDefault", "Default", group_sel_colour, 0,
					&NotebookWindow::on_view_set_selection_colour, prefs.selection_colour == "#ccc");
				menu_add_radioaction("SelectionColourCustom", "Custom...", group_sel_colour, 1,
					&NotebookWindow::on_view_set_selection_colour, prefs.selection_colour != "#ccc");
			menu_add_action("MenuOutputSize", "Output size");
				Gtk::RadioAction::Group group_output_size;
				menu_add_radioaction("OutputSizeSmall", "Small", group_output_size, -1,
					&NotebookWindow::on_view_set_output_size, prefs.font_step == -1);
				menu_add_radioaction("OutputSizeMedium", "Medium (default)", group_output_size, 0,
					&NotebookWindow::on_view_set_output_size, prefs.font_step == 0);
				menu_add_radioaction("OutputSizeLarge", "Large", group_output_size, 2,
					&NotebookWindow::on_view_set_output_size, prefs.font_step == 2);
				menu_add_radioaction("OutputSizeExtraLarge", "ExtraLarge", group_output_size, 4,
					&NotebookWindow::on_view_set_output_size, prefs.font_step == 4);
			menu_add_action("StyleResetToDefaults", "Reset to defaults", &NotebookWindow::on_view_reset_to_defaults);

	auto menu_evaluate = menu_add_action("MenuEvaluate", "_Evaluate");
		menu_add_action("EvaluateCell", "Evaluate cell", &NotebookWindow::on_run_cell, "<shift>Return");
		menu_add_action("EvaluateAll", "Evaluate all", Gtk::Stock::GO_FORWARD, &NotebookWindow::on_run_runall);
		menu_add_action("EvaluateToCursor", "Evaluate to cursor", Gtk::Stock::GOTO_LAST, &NotebookWindow::on_run_runtocursor);
		menu_add_action("EvaluateStop", "Stop", Gtk::Stock::STOP, &NotebookWindow::on_run_stop);

	auto menu_kernel = menu_add_action("MenuKernel", "_Kernel");
		menu_add_action("KernelRestart", "Restart", Gtk::Stock::REFRESH, &NotebookWindow::on_kernel_restart, "<ctrl><alt>R");

	auto menu_compare = menu_add_action("MenuCompare", "_Compare");
		menu_add_action("CompareFile", "Compare to file", &NotebookWindow::on_compare_to_file);
		menu_add_action("MenuGit", "Compare with Git");
		menu_add_action("GitLatest", "Latest commit", &NotebookWindow::on_compare_git_latest);
		menu_add_action("GitChoose", "Select commit from list", &NotebookWindow::on_compare_git_choose);
		menu_add_action("GitSpecific", "Manually enter commit hash", &NotebookWindow::on_compare_git_specific);
		menu_add_action("GitSelectExecutable", "Select Git Executable", &NotebookWindow::on_compare_select_git_executable);

	auto menu_help = menu_add_action("MenuHelp", "_Help");
		menu_add_action("HelpAbout", "About Cadabra", Gtk::Stock::ABOUT, &NotebookWindow::on_help_about);
		menu_add_action("HelpContext", "Contextual Help", Gtk::Stock::HELP, &NotebookWindow::on_help);
		menu_help_register = menu_add_action("HelpRegister", "Register", &NotebookWindow::on_help_register);
		menu_help_register->set_sensitive(!prefs.is_registered);

	if (read_only) {
		menu_edit->set_sensitive(false);
		menu_view->set_sensitive(false);
		menu_evaluate->set_sensitive(false);
	}

	Glib::ustring ui_info = R"UI_INFO(
		<ui>
		  <menubar name='MenuBar'>
		    <menu action='MenuFile'>
		      ...
		      <menuitem action='FileExportHtmlSegment' />
		      <menuitem action='FileExportLaTeX' />
		      <menuitem action='FileExportPython' />
		      <separator />
		    ...
		    <menu action='MenuEdit'>
		      ...
		      <separator />
		      <menuitem action='EditMakeCellLaTeX' />
            ...
            <separator />
            <menu action='MenuStyle'>
		        <menuitem action='StyleFont' />
              <menu action='MenuSyntaxHighlighting'>
                <menuitem action='SyntaxHighlightingOn' />
                <menuitem action='SyntaxHighlightingOff' />
                <menuitem action='SyntaxHighlightingCustomize' />
              </menu>
		        <menuitem action='StyleTextColour' />
		        <menu action='MenuSelectionColour'>
		          <menuitem action='SelectionColourDefault' />
		          <menuitem action='SelectionColourCustom' />
		        </menu>
		        <menu action='MenuOutputSize'>
		          <menuitem action='OutputSizeSmall' />
		          <menuitem action='OutputSizeMedium' />
		          <menuitem action='OutputSizeLarge' />
		          <menuitem action='OutputSizeExtraLarge' />
		        </menu>
              <menuitem action='StyleResetToDefaults' />
		      </menu>
		    </menu>
		    ...
		    <menu action='MenuCompare'>
		      ...
		      <menu action='MenuGit'>
		        <menuitem action='GitLatest' />
		        <menuitem action='GitChoose' />
		        <menuitem action='GitSpecific' />
		        <separator />
		        <menuitem action='GitSelectExecutable' />
		      </menu>
		    </menu>
...
// Found hidden somewhere in the middle of gtk/gtkfontbutton.c
static gchar*
pango_font_description_to_css(PangoFontDescription* desc)
{
	...
	{
		g_string_append_printf(s, "font-size: %dpx", pango_font_description_get_size(desc) / PANGO_SCALE);
	}
...
void NotebookWindow::load_css()
{
	...
	bool old_selectors = (GTKMM_MAJOR_VERSION <= 3 && GTKMM_MINOR_VERSION <= 20);

	std::string data = R"CSS_DATA(
		*:selected {
		...
		*:focus {
		...
		#CodeInput %selected% {
			background-color: %selection_colour%;
		}
	...
	auto font_desc = pango_font_description_from_string(prefs.font.c_str());
	auto font_css = Glib::convert_return_gchar_ptr_to_stdstring(pango_font_description_to_css(font_desc));
	pango_font_description_free(font_desc);

	replace_all(data, "%font_properties%", font_css);
	replace_all(data, "%text_colour%", prefs.highlight ? "black" : prefs.text_colour);
	replace_all(data, "%selected%", old_selectors ? " *:selected" : "selection");
	replace_all(data, "%selection_colour%", prefs.selection_colour);
	...
	set_output_size(prefs.font_step);
}
...
void NotebookWindow::on_view_set_console_vis(Glib::RefPtr<Gtk::RadioAction> opt)
{
	if (!opt->get_active())
		return;

	int vis = opt->get_current_value();
...
void NotebookWindow::update_cell(const DTree&, DTree::iterator it)
{
	...
	for (unsigned int i = 0; i < canvasses.size(); ++i) {
		...
		if (it->cell_type == DataCell::CellType::python || it->cell_type == DataCell::CellType::latex) {
			...
			vc.inbox->queue_draw();
			if (prefs.highlight) {
				vc.inbox->disable_highlighting();
				vc.inbox->enable_highlighting(it->cell_type, prefs);;
			}
		}
	}
...
void NotebookWindow::refresh_all_cells()
{
	for (auto beg = doc.begin(), end = doc.end(); beg != end; ++beg) {
		update_cell(doc, beg);
	}
}
...
void NotebookWindow::on_compare_select_git_executable()
{
...
void NotebookWindow::on_compare_to_file()
{
	std::string filename;
...
void NotebookWindow::on_compare_git_latest()
{
	try {
...
void NotebookWindow::on_compare_git_choose()
{
#ifndef _MSC_VER
	try {
...
void NotebookWindow::on_compare_git_specific()
{
	try {
		std::string commit_hash;
		Gtk::Entry entry;
...
void NotebookWindow::set_output_size(int num)
{
...
void NotebookWindow::on_view_set_output_size(Glib::RefPtr<Gtk::RadioAction> opt)
{
	if (!opt->get_active())
		return;

	int font_step = opt->get_current_value();
	if (prefs.font_step != font_step) {
		prefs.font_step = font_step;
		prefs.save();
		set_output_size(font_step);
	}
}

void NotebookWindow::on_view_set_text_colour()
{
	Gtk::ColorChooserDialog dlg("Select text colour");
	dlg.set_transient_for(*this);
	if (dlg.run() == Gtk::RESPONSE_OK) {
		prefs.text_colour = dlg.get_rgba().to_string();
		prefs.save();
		load_css();
	}
}

void NotebookWindow::on_view_set_selection_colour(Glib::RefPtr<Gtk::RadioAction> opt)
{
	if (!opt->get_active())
		return;

	int selection = opt->get_current_value();

	if (selection == 0) {
		prefs.selection_colour = "#ccc";
		prefs.save();
		load_css();
	}
	else if (selection == 1) {
		Gtk::ColorChooserDialog dlg("Select selection colour");
		dlg.set_transient_for(*this);
		if (dlg.run() == Gtk::RESPONSE_OK) {
			prefs.selection_colour = dlg.get_rgba().to_string();
			prefs.save();
			load_css();
		}
		else {
			// If default was previously selected then reselect it
			if (prefs.selection_colour == "#ccc") {
				opt->set_current_value(0);
			}
		}
	}
}

void NotebookWindow::on_view_set_font()
{
	Gtk::FontChooserDialog dlg("Select font", *this);
	dlg.set_transient_for(*this);
	dlg.set_font(prefs.font);
	if (dlg.run() == Gtk::RESPONSE_OK) {
		prefs.font = dlg.get_font();
		prefs.save();
		load_css();
	}
}

void NotebookWindow::on_view_set_syntax_hl(Glib::RefPtr<Gtk::RadioAction> opt)
{
	if (!opt->get_active())
		return;
	bool is_on = (opt->get_current_value() == 1);
	if (prefs.highlight == is_on)
		return;
	prefs.highlight = is_on;
	prefs.save();

	actiongroup->get_action("StyleTextColour")->set_sensitive(!is_on);
...
void NotebookWindow::on_view_customize_syntax_hl()
{
	cadabra::ChooseColoursDialog dlg(prefs, *this);
	dlg.set_transient_for(*this);
	dlg.run();
	prefs.save();
}

void NotebookWindow::on_view_reset_to_defaults()
{
