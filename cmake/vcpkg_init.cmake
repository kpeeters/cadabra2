...
if (${N_CONFIGURATION_TYPES} EQUAL 1)
	...
	message(STATUS "CMAKE_BUILD_TYPE set to ${CMAKE_BUILD_TYPE} as this is the only supported configuration")
endif()

# Attempts to find the vcpkg.cmake toolchain file and include it
# if it has not been supplied.
if (NOT VCPKG_TOOLCHAIN)
	message(STATUS "VCPKG_TOOLCHAIN not defined, attempting to find vcpkg.cmake")
...
if (VCPKG_TOOLCHAIN)
	...
	function(vcpkg_find_file VAR NAME PATHS EXTENSION)
		set(TMPVAR "")
		foreach (DIR ${PATHS})
			...
			if(CMAKE_BUILD_TYPE MATCHES "^Debug$" OR NOT DEFINED CMAKE_BUILD_TYPE)
				if ("${TMPVAR}" STREQUAL "") # name-d.lib
					file(GLOB TMPVAR "${DIR}/${LIBNAME}-d.${EXTENSION}")
				endif()
				if ("${TMPVAR}" STREQUAL "") # named.lib
					file(GLOB TMPVAR "${DIR}/${LIBNAME}d.${EXTENSION}")
				endif()
				if ("${TMPVAR}" STREQUAL "") # name-<version-number>-d.lib
					file(GLOB TMPVAR "${DIR}/${LIBNAME}-[0-9]*-d.${EXTENSION}")
				endif()
				if ("${TMPVAR}" STREQUAL "") # name-<version-number>d.lib
					file(GLOB TMPVAR "${DIR}/${LIBNAME}-[0-9]*-d.${EXTENSION}")
				endif()
				...
				if ("${TMPVAR}" STREQUAL "") # name-d-<version-number>.lib
					file(GLOB TMPVAR "${DIR}/${LIBNAME}-d-[0-9]*.${EXTENSION}")
				endif()
			...
			if ("${TMPVAR}" STREQUAL "") # name.lib
				file(GLOB TMPVAR "${DIR}/${LIBNAME}.${EXTENSION}")
			endif()
			if ("${TMPVAR}" STREQUAL "") # name-<version-number>.lib
				file(GLOB TMPVAR "${DIR}/${LIBNAME}-[0-9]*.${EXTENSION}")
			endif()
			if ("${TMPVAR}" STREQUAL "") # name<version-number>.lib
				file(GLOB TMPVAR "${DIR}/${LIBNAME}[0-9]*.${EXTENSION}")
			endif()
	...
	function(vcpkg_find_library VAR LIBNAME)
		...
		if ("${TMPVAR}" STREQUAL "")
			...
			message(STATUS "${VAR} already set to ${TMPVAR}")
			return()
		endif()
		...
		if ("${TMPVAR}" STREQUAL "")
			...
			message(SEND_ERROR "Could NOT find library ${LIBNAME}")
		else()
			...
			message(STATUS "${LIBNAME} found at ${TMPVAR}")
		endif()
	endfunction()
endif()