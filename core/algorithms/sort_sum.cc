...
sort_sum::sort_sum(const Kernel& k, Ex& e, unsigned int runsize)
	: Algorithm(k, e), runsize(runsize)
	{
	}
...
// insertionSort on first num elements generated by iterator step
...
bool sort_sum::timSort(iterator& st)
	...
	// This reduces to insertionSort on sums of fewer than 32 terms. (for default runsize)
    ...
    // minRun is computed to lie in (16, 32) (for default runsize)
	// 
...
unsigned int sort_sum::calcMinRun(unsigned int n)
	 ...
	 * 
	 * Python's timSort uses RUN = 64. Set to 32 here by default.
	 * 
	 */
	unsigned int r = 0;
	while (n >= runsize) {
